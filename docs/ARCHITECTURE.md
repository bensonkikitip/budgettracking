...
| `/account/[id]/reconcile` | `account/[id]/reconcile.tsx` | Post-import reconciliation screen. Lists side-by-side pairs of manual transactions that likely overlap imported ones (same amount, date ±1 day). Per-pair actions: **Merge** (deletes the manual row, optionally carries its manually-set category to the imported row) or **Keep Both** (dismisses from list, no DB write). Empty state shown when all pairs are resolved. Reached from the reconciliation CTA card on the import done screen. |
...
- E2E flows are in `maestro/` and run against a live simulator with `maestro test maestro/<flow>.yaml`. They test complete user journeys (onboarding, import, categorization, rules, backup, period navigation, and PDF import) against the real app — no mocking. Requires Maestro 2.5.0+ (install: `curl -Ls "https://get.maestro.mobile.dev" | bash`). Because Maestro cannot drive the iOS document picker for arbitrary file types, fixture data is injected via dev-only buttons in the import screen (`__DEV__` only): "Load … .csv" buttons feed fixture CSV data (lives in [`src/test/fixtures.ts`](../src/test/fixtures.ts)), and a "Load sample_citi_statement.pdf" button writes a base64-encoded fixture PDF to the cache directory and calls the real `extractPdfItems` native function — exercising the full native → parser pipeline in `pdf_import_flow.yaml`. Synthetic sample PDFs for onboarding are under `maestro/fixtures/`. See the ship checklist in [CLAUDE.md](../CLAUDE.md) for which flows must pass before each release.

**Coverage map:**

| Folder | What's covered |
|---|---|
| `tests/db/migrations.test.ts` | All 15 migrations apply cleanly; FKs are on; `applied_rule_id` has no FK (regression for v4.2.0); `source_is_manual` column present after migration 15. |
...
| `tests/db/source-tracking.test.ts` | `source_is_manual` set correctly on insert (1 for manual, 0 for imported); migration backfill for pre-v15 rows; `findReconciliationCandidates` finds same-amount/close-date pairs; `mergeManualIntoImported` deletes manual row and copies category when manually set. |
